...
def convert_status(tree, node):
    if tree.status[node] is True:
        tree.status[node] = False
    else:
        tree.status[node] = True

def convert_power(tree, node, power):
    tree.authority[node] = min(20, power)
...
def convert_parent(tree, node1, node2):
    ...
    if node1 == node2 or pnode1 == pnode2:
        return
    if len(pnode1.children) == 2:
        ...
        pnode1.children[0] = node2
    tree.idx_to_node[node2.value][0] = pnode1.value
    ...
    if len(pnode2.children) == 2:
        ...
        pnode2.children[0] = node1
    tree.idx_to_node[node1.value][0] = pnode2.value
